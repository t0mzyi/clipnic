...
export class AuthService {
  ...
  static async syncUser(supabaseId: string, email: string, name?: string, avatarUrl?: string, role: string = 'user') {
    const { data, error } = await supabase
      ...
      .select('id, email, name, avatar_url, role, discord_id, discord_verified, youtube_verified, youtube_handle, youtube_channels, instagram_verified, instagram_handle, is_blocked, onboarding_completed, onboarding_step')
      .single();
